...
function startUp() {
    ...
    $('body').keydown(controlKeysDown);
    $('body').keyup(controlKeysUp);
}

function controlKeysDown(e) {
...
function init() {
	...
	scene.add(camera);
	camera.sMatrix = new THREE.Matrix4();
...
function createWorld() {
	...
	//floor
	var floorTexture = new THREE.ImageUtils.loadTexture(texDir+'wood-floor.jpg');
	...
	var pillarTexture = new THREE.ImageUtils.loadTexture(texDir+'wood-wall.jpg');
	...
	var ceilingTexture = new THREE.ImageUtils.loadTexture(texDir+'wood-wall.jpg');
	...
	var wallTexture = new THREE.ImageUtils.loadTexture(texDir+'wood-wall.jpg');
...
function createSkybox() {
	...
	var materialArray = [];
	materialArray.push(new THREE.MeshBasicMaterial( { map: THREE.ImageUtils.loadTexture( texDir+box+'/right.jpg') }));
	materialArray.push(new THREE.MeshBasicMaterial( { map: THREE.ImageUtils.loadTexture( texDir+box+'/left.jpg'	) }));
	materialArray.push(new THREE.MeshBasicMaterial( { map: THREE.ImageUtils.loadTexture( texDir+box+'/top.jpg'	) }));
	materialArray.push(new THREE.MeshBasicMaterial( { map: THREE.ImageUtils.loadTexture( texDir+'grass.png'		) }));
	materialArray.push(new THREE.MeshBasicMaterial( { map: THREE.ImageUtils.loadTexture( texDir+box+'/front.jpg') }));
	materialArray.push(new THREE.MeshBasicMaterial( { map: THREE.ImageUtils.loadTexture( texDir+box+'/back.jpg'	) }));
	for (var i = 0; i < 6; i++)
...
function moveSnake(delta){
	...
	var up = (((keyboard.pressed(gummywormPrefs.down) || keyboard.pressed("down")) && gummywormPrefs.inverted) || (keyboard.pressed(gummywormPrefs.up) || keyboard.pressed("up")) && !gummywormPrefs.inverted) && !lock.up;
	var down = (((keyboard.pressed(gummywormPrefs.up) || keyboard.pressed("up")) && gummywormPrefs.inverted) || (keyboard.pressed(gummywormPrefs.down) || keyboard.pressed("down")) && !gummywormPrefs.inverted) && !lock.down;
	var left = keyboard.pressed(gummywormPrefs.left) || keyboard.pressed("left") && !lock.left;
	var right = keyboard.pressed(gummywormPrefs.right) || keyboard.pressed("right") && !lock.right;
	var rotLeft = keyboard.pressed("Q") && false;
	var rotRight = keyboard.pressed("E") && false;

	// if (keyboard.pressed("shift")) {
	// 	moveDistance*=4;
	// }

	var rotDegreesAtATime = 10;

	if (move.left && !lock.left) {
		// lock.left = true;
		lockAll();
		console.log("left");

		// var degrees = 0;
		// var slowRotate = setInterval(function() {
		// var rotation_matrix = new THREE.Matrix4().makeRotationY(rotDegreesAtATime*ATR);
		var rotation_matrix = new THREE.Matrix4().makeRotationY(90*ATR);
		snake.sMatrix.multiply(rotation_matrix);
		snake.head.rotation.setEulerFromRotationMatrix(snake.sMatrix);
		// 	degrees+=rotDegreesAtATime;
		// 	if (degrees >= 90)
		// 		clearInterval(slowRotate);
		// }, 1);

		// var rotation_matrix_camera = new THREE.Matrix4().makeRotationY(-90*ATR);
		// camera.sMatrix.multiply(rotation_matrix_camera);
		// camera.rotation.setEulerFromRotationMatrix(camera.sMatrix);

		// camera.translateX(-800);
		// camera.translateZ(-800);

		// var position = camera.position.clone();
		// var target = { x : 0, y: 160, z: 800 };
		...
		// var banana = setInterval(function() {
			...
			// var translate_matrix_camera = new THREE.Matrix4().translateZ(-800);
			// rotation_matrix_camera = new THREE.Matrix4().makeRotationY(1*ATR);
			// // translate_matrix_camera = new THREE.Matrix4().translateZ(800);
			// camera.sMatrix.multiply(translate_matrix_camera);
			// camera.sMatrix.multiply(rotation_matrix_camera);
			// camera.sMatrix.multiply(translate_matrix_camera);
			// camera.rotation.setEulerFromRotationMatrix(camera.sMatrix);
			...
			// if (angle < 45)
				// camera.translateX(800/90*Math.sin(angle*ATR))
		// camera.translateZ(800/90)
			// else if (angle < 90)
			// camera.translateX(800/90*Math.cos(angle*ATR));
			// camera.translateZ(800/90*Math.sin(angle*ATR));
	...
	if (move.up && !lock.up) {
		...
		console.log("up");
		lockAll();
		// var rotation_matrix = new THREE.Matrix4().makeRotationX(90*ATR);
		// snake.sMatrix.multiply(rotation_matrix);
		// snake.head.rotation.setEulerFromRotationMatrix(snake.sMatrix);
		...
		// var degrees = 0;
		// var slowRotate = setInterval(function() {
		// 	var rotation_matrix = new THREE.Matrix4().makeRotationX(rotDegreesAtATime*ATR);
		// 	snake.sMatrix.multiply(rotation_matrix);
		// 	snake.head.rotation.setEulerFromRotationMatrix(snake.sMatrix);
		// 	degrees+=rotDegreesAtATime;
		// 	if (degrees >= 90)
...
function update() {
	...
	var hitSelf = false;
	for (var i = 4; i < snake.traces.length; i += 1) {
...
var opponents = [];
var opponentIds = {};
var socket = io.connect('http://gummyworm.herokuapp.com:80');
